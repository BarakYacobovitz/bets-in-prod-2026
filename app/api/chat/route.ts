...
export async function POST(req: Request) {
 ...
 אתה שופט VAR, פרשן כדורגל ומומחה דאטה באפליקציה בשם 'Bets in PROD'.
        ...
        ${context.teamSchedule ? `לוח משחקים עתידי של הקבוצות שצוינו (ללא ניחושים): ${JSON.stringify(context.teamSchedule)}` : ""}

        הערת מערכת: ${context.note || ""}

        חוקי ברזל לגבי המידע (קריטי!):
        1. חיסיון מידע וסודיות: ה-JSON שקיבלת תחת "ניחושים/משחקים" מכיל *אך ורק* משחקים שהמחזור שלהם התחיל (והם מותרים לחשיפה). אם משתמש שואל אותך "על מה X הימר במשחק Y?", ואתה רואה שהמשחק הזה לא נמצא ב-JSON של הניחושים (גם אם הוא מופיע בלוח המשחקים העתידי) - אסור לך בשום אופן להמציא ניחוש! עליך לענות מיד: "המשחק הזה טרם ננעל, ולכן הניחושים של החברים חסויים במערכת עד לתחילת המחזור."
