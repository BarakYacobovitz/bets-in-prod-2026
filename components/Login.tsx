...
export default function Login() {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      // 2. בודקים אם זה משתמש חדש (דרך גוגל או לפי המסמך שלו ב-DB)
  ...
  return (
    <div className="relative min-h-screen w-full flex items-center justify-center overflow-hidden" dir="rtl">
      ...
      {/* קופסת ההתחברות (Glassmorphism) */}
<div className="relative z-10 bg-slate-900/20 backdrop-blur-sm p-8 md:p-12 rounded-[2.5rem] border border-white/5 shadow-[0_0_60px_rgba(0,0,0,0.4)] max-w-md w-full mx-4 text-center flex flex-col items-center animate-fade-in-up">       
        ...
        {/* הצגת הודעת השגיאה במידה והשחקן נחסם */}
        {errorMsg && (
          <div className="w-full bg-rose-950/80 border border-rose-500/50 text-rose-300 text-sm font-bold p-4 rounded-xl mb-6 shadow-inner animate-fade-in-up">
            {errorMsg}
          </div>
        )}
