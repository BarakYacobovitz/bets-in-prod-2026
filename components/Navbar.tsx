...
export default function Navbar() {
  ...
  useEffect(() => {
  const saved = localStorage.getItem("isLargeText") === "true";
  ...
  localStorage.setItem("isLargeText", String(nextValue));
  ...
  return (
    <>
      <style dangerouslySetInnerHTML={{__html: `
        /* מגדיל את מידת הבסיס של כל האתר ב-15% */
        html.large-text {
             ...
             <div className="flex items-center gap-2 bg-slate-900 border border-slate-700 p-1 pl-1.5 md:p-1.5 md:pl-4 rounded-full shadow-inner">
              {/* --- כפתור נגישות --- */}
                <button 
                  ...
                  className={`w-6 h-6 md:w-8 md:h-8 rounded-full flex items-center justify-center text-[10px] md:text-xs font-black transition-all shrink-0 ${isLargeText ? 'bg-amber-500 text-white' : 'bg-slate-700 text-slate-300 hover:bg-slate-600'}`}
                >
                  {isLargeText ? 'Aa-' : 'Aa+'}
                </button>
                {photoUrl ? (
